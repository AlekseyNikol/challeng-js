...
1. Объявите переменную user и инициализируйте ее следующими свойствами: name 'Лев Толстой', login 'leo_tolstoy', reputation 97, total 900.
...
1. копируйте и вставьте в свою работу следующий код:
...
function convertArrayToObj2(messages) {
    let array = [];
    for (let i = 0; i < messages.length; i++) {
        let obj = {};
        let key = messages[i][0];
        let value = messages[i][1];
        obj[key] = value;
        array.push(obj);
    }
...
console.log(convertArrayToObj2(messages));

console.log('\n')

/*
function convertArrayToObj(messages) {
  let array = [];
  for (let i = 0; i < messages.length; i ++) {
    let obj = {};
    let key = messages[i][0];
...
console.log(convertArrayToObj(messages));
*/
